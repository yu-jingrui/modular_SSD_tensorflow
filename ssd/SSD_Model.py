...
class SSDModel():
    ...
    def compute_jaccard(self, gt_bboxes, anchors):
        ...
        inter_area = h * w
        anchors_area = (anchors[:, :, 3] - anchors[:, :, 1]) * \
            (anchors[:, :, 2] - anchors[:, :, 0])
        gt_bboxes_area = (gt_bboxes[:, :, 3] - gt_bboxes[:, :, 1]) * \
    ...
    def __match_no_miss(self, gt_anchor_labels, gt_anchor_bboxes, gt_anchor_scores, jaccard, gt_labels, gt_bboxes,
                        num_anchors):
        ...
        def body(i, gt_anchors_labels, gt_anchors_bboxes, gt_anchors_scores):
            ...
            gt_anchors_labels = tf.where(
                new_mask, new_labels, gt_anchors_labels)
            ...
            gt_anchors_bboxes = tf.where(
                new_mask, new_bboxes, gt_anchors_bboxes)
            ...
            gt_anchors_scores = tf.where(
                new_mask, new_scores, gt_anchors_scores)
    ...
    def __match_no_labels(self, gt_anchor_labels, gt_anchor_bboxes, gt_anchor_scores, jaccard, matching_threshold,
                          gt_labels, gt_bboxes, num_anchors):
        # For images without labels, just return all zero tensors

        return gt_anchor_labels, gt_anchor_bboxes, gt_anchor_scores

    def __match_with_labels(self, gt_anchor_labels, gt_anchor_bboxes, gt_anchor_scores, jaccard, matching_threshold,
                            gt_labels, gt_bboxes, num_anchors):
        # debugging info
        ...
        gt_anchor_bboxes = tf.where(mask, tf.gather(
            gt_bboxes, mask_inds), gt_anchor_bboxes)
    ...
    def match_achors(self, gt_labels, gt_bboxes, matching_threshold=0.5):
        ...
        gt_anchor_labels, gt_anchor_bboxes, gt_anchor_scores = tf.cond(tf.equal(n__glabels, 0),
                                                                       lambda: self.__match_no_labels(gt_anchor_labels,
                                                                                                      gt_anchor_bboxes,
                                                                                                      gt_anchor_scores,
                                                                                                      jaccard,
                                                                                                      matching_threshold,
                                                                                                      ...
                                                                                                      gt_bboxes,
                                                                                                      num_anchors),
                                                                       lambda: self.__match_with_labels(
        ...
        gt_anchor_labels, gt_anchor_bboxes, gt_anchor_scores = self.__convert2layers(gt_anchor_labels, gt_anchor_bboxes,
                                                                                     gt_anchor_scores)
    ...
    def __convert2layers(self, gclasses, glocalisations, gscores):
        ...
        for i in range(len(anchors)):
            ...
            gt_anchor_labels.append(tf.reshape(
                gclasses[start:end], anchor_shape))
            gt_anchor_scores.append(tf.reshape(
                gscores[start:end], anchor_shape))
            gt_anchor_bboxes.append(tf.reshape(
                glocalisations[start:end], anchor_shape + [4]))
    ...
    def decode_bboxes_all_layers(self, localizations):
        ...
        for i in range(len(localizations)):
            decoded_bboxes.append(self.decode_bboxes_layer(
                localizations[i], all_anchors[i]))
